...
from compiler.llm import m1_provenance_graph_projection_adapter as adapter  # noqa: E402
...
FIXTURE_PATH = (
    ...
    / "synthetic_provenance_graph_projection_v0.1.json"
)
FIXTURE_ID = "fixture_provenance_graph_projection_001"
...
class M1ProvenanceGraphProjectionAdapterTests(unittest.TestCase):
    ...
    def test_fixture_is_declaration_only_with_explicit_observed_basis(self):
        ...
        for forbidden in ('"raw_graph"', '"payload"', '"labels"', '"secret"'):
            self.assertNotIn(forbidden, text)

    def test_missing_authority_rejects(self):
        fixture = load_fixture()
        with self.assertRaises(
            adapter.M1ProvenanceGraphProjectionAdapterError
        ) as context:
            adapter.adapt_provenance_graph_projection(
    ...
    def test_happy_path_returns_schema_valid_unminted_envelope(self):
        ...
        result = adapter.adapt_provenance_graph_projection(
            descriptor,
            repo_root=REPO_ROOT,
            authority=authority,
    ...
    def test_path_uri_raw_bytes_labels_and_unknown_fail_closed(self):
        ...
        unknown_fixture["graph"]["unknown"] = 1
        descriptor, registry, authority = execution_bundle(unknown_fixture)
        with self.assertRaises(
            adapter.M1ProvenanceGraphProjectionAdapterError
        ) as context:
            adapter.adapt_provenance_graph_projection(
    ...
    def test_modality_mismatch_and_dangling_edge_fail_closed(self):
        mismatch = load_fixture()
        mismatch["source_metadata"]["epistemic_modality"] = "derived"
        dangling = load_fixture()
        dangling["graph"]["edges"][0]["to_node_id"] = "node_missing"
        for fixture, expected_code in (
            (mismatch, "modality_mapping"),
    ...
    def test_authority_elevation_and_activation_ledger_fail_closed(self):
        elevated = load_fixture()
        elevated["admission_state"] = "admitted_under_separate_authority"
        descriptor, registry, authority = execution_bundle(elevated)
        with self.assertRaises(
            adapter.M1ProvenanceGraphProjectionAdapterError
        ) as context:
            adapter.adapt_provenance_graph_projection(
        ...
        fixture = load_fixture()
        descriptor, registry, authority = execution_bundle(fixture)
        authority["execute_ledger"] = {"remaining": 1}
        with self.assertRaises(
            adapter.M1ProvenanceGraphProjectionAdapterError
        ) as context:
            adapter.adapt_provenance_graph_projection(
