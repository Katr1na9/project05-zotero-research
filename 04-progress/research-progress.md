...
- 在结果前冻结 AFA-VOI 协议，并实现 Myopic 与 Rollout-H3 同接口领域适配；C07-C10 均 180/180 达标，但平均成功成本 `4.9722`，比 M2 的 `4.5333` 高 `0.4389`。该结果不是 NOCTA/WinRegRL 官方复现。
- 完成 M2 单权重 ±25% 敏感性：16 个变体均保持 success=1.0，13 个与原策略完全同序，另 3 个成本增加 `0.0222`；支持局部稳定，不支持全局最优。
- 完成三档 G 阈值与 OR/AND 审计：真实 C07-C10 每节点只有一条 required claim，OR/AND 不可识别；C01-C06 多 claim 开发压力中，AND 使 M2 success 从 `0.8000` 降至 `0.4963`，确认内部代理需要人工校准。
- 新增当前权威文件索引、贡献边界 v0.2、论文/专利叙事 v0.2、Reviewer 回复矩阵和专利一致性附录；旧 M3a/三案例辅稿保留为历史档案但停止作为写作入口。
- 当前最高优先级未完成项仍是双人盲标；模板保持 `awaiting_annotations`，没有以代码或 LLM 标签冒充人工结果。

## 2026-07-11：非短视必要性通过，DQN 必要性不通过

- 实现 `run_nonmyopic_gate.py`，构造 192 个冻结参数环境，覆盖解锁深度 1–4、预算余量 0/1、诱饵数量 1/3/6/10、即时收益 0.5/1/2 和链路可靠性 1.0/0.8；10 个配对 seed，共 7,680 episodes。
- Gate A 通过：166 个环境出现稳定冲突，DP 相对冻结 M2 的平均 success 优势 `0.6208`；Depth-2 在深度 1 上修复 `0.4000`。
- Gate B 不通过：DP 虽比 Depth-2 高 `0.3448` success，但冷启动中位/p95 仅 `0.7692/83.9598 ms`，最大展开 `23,892` 状态，未达到 `100 ms` 或 `100,000` 状态门槛。
- 决策：`use_lightweight_nonmyopic_planning_no_dqn`。当前需要更深规划，但没有证据支持启动 DQN/Dueling DQN；RL 降为未来工作或附录对照。
- 边界：这是结构化存在性诊断，不估计真实攻击调查中的非短视状态发生率，也不证明“RL 无效”。
