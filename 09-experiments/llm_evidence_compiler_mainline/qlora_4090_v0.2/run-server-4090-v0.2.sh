...
readonly CONTRACT="${BUNDLE_ROOT}/09-experiments/llm_evidence_compiler_mainline/contracts/qwen25-qlora-4090-training-contract-v0.2.json"
readonly CONFIG="${BUNDLE_ROOT}/09-experiments/llm_evidence_compiler_mainline/qlora_4090_v0.2/training-config-v0.2.json"
readonly AUTHORITY="${BUNDLE_ROOT}/09-experiments/llm_evidence_compiler_mainline/contracts/authority-lock-v0.32.json"
...
for path in "${RUN_ROOT}" "${BUNDLE_ROOT}" "${VENV_ROOT}" "${CONTRACT}" "${CONFIG}" \
  "${AUTHORITY}" "${EXECUTE_SCRIPT}" "${PAIR_ROOT}" "${PREPARATION_AUDIT}" "${SMOKE_AUDIT}"; do
  case "${path}" in
    "${RUN_ROOT}"|"${RUN_ROOT}"/*) ;;
...
for required in "${RUNTIME_READY_MARKER}" "${VENV_ROOT}/bin/python" "${CONTRACT}" \
  "${CONFIG}" "${AUTHORITY}" "${EXECUTE_SCRIPT}" "${PREPARATION_AUDIT}" \
  "${SMOKE_AUDIT}" "${PAIR_ROOT}/train.jsonl.gz" \
  "${PAIR_ROOT}/training-validation.jsonl.gz"; do
  if [[ ! -f "${required}" ]]; then
...
if [[ "${1:-primary}" != "primary" ]]; then
  echo "usage: $0 primary" >&2
  exit 2
fi
