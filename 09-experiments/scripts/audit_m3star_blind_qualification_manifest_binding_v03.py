#!/usr/bin/env python3
"""Bind v0.3 staged-qualified opaque identities to the final blind manifest."""
...
def load_staged_auditor() -> Any:
    ...
    spec = importlib.util.spec_from_file_location(
        "staged_qualification_auditor_v03", path
    )
...
def validate_binding(
    ...
    base_amendment = load_json(base_amendment_path)
    stopping_amendment = load_json(stopping_amendment_path)
    ...
    staged_audit = staged.validate_qualification_report(
        ...
        base_amendment_path,
        stopping_amendment,
        stopping_amendment_path,
    )
    if staged_audit.get("status") != "qualification_complete":
        raise ValueError("Staged qualification is not complete under v0.3")
    ...
    for field in (
        "amendment_id",
        "amendment_sha256",
        "stopping_amendment_id",
        "stopping_amendment_sha256",
        ...
        if readiness.get(field) != staged_audit.get(field):
            raise ValueError(
                f"Qualification readiness {field} does not match the v0.3 report audit"
            )
    ...
    return {
        "audit_id": "project05-m3star-blind-qualification-manifest-binding-v0.3",
        "status": "qualification_manifest_binding_complete",
        "checked_utc": utc_now(),
        "staged_amendment_id": base_amendment["amendment_id"],
        "staged_amendment_sha256": sha256(base_amendment_path),
...
def main() -> None:
    ...
    audit = validate_binding(
        args.base_amendment.resolve(strict=True),
        args.stopping_amendment.resolve(strict=True),
