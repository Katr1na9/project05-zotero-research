#!/usr/bin/env python3
"""v0.3 one-shot runner for the sealed M3* external evaluation."""
...
POWER_DESIGN_RELATIVE_PATH = Path(
    "09-experiments/results/m3star_final_blind_power_design_v0.2/power_design.json"
)
...
STOPPING_AMENDMENT_RELATIVE_PATH = Path(
    "04-progress/m3star-final-blind-data-intake-v0.1-20260719/"
    "sample-size-and-stopping-amendment-v0.3.json"
)
STAGED_QUALIFICATION_AUDITOR_RELATIVE_PATH = Path(
...
QUALIFICATION_BINDING_SCHEMA_RELATIVE_PATH = Path(
    "09-experiments/data_schema/"
    "m3star_blind_qualification_manifest_binding_v03.schema.json"
)
QUALIFICATION_BINDING_AUDITOR_RELATIVE_PATH = Path(
    "09-experiments/scripts/audit_m3star_blind_qualification_manifest_binding_v03.py"
)
MINIMUM_VALID_COMPLETE_CASES = 59
...
def validate_protocol_document(protocol: dict[str, Any]) -> None:
    if protocol.get("protocol_id") != "project05-m3star-final-blind-protocol-v0.3":
        raise ValueError("Final-blind protocol v0.3 is required")
    ...
    if sample_size.get("minimum_valid_complete_cases") != MINIMUM_VALID_COMPLETE_CASES:
        raise ValueError("Minimum valid independent-case count differs from 59")
    ...
    for field, expected_relative_path in (
        ...
        ("stopping_amendment", STOPPING_AMENDMENT_RELATIVE_PATH),
        ("staged_auditor", STAGED_QUALIFICATION_AUDITOR_RELATIVE_PATH),
    ...
    if qualification_gate.get("minimum_qualified_cases") != MINIMUM_VALID_COMPLETE_CASES:
        raise ValueError("Qualification-gate minimum differs from 59")
...
def validate_qualification_artifacts(
    ...
    stopping_path = resolve_repo_relative_path(
        gate["stopping_amendment_path"],
        "qualification_gate.stopping_amendment_path",
    )
    stopping = load_json(stopping_path)
    ...
    if (
        ...
        raise ValueError("Qualified independent-case count must be between 59 and 95")
    ...
    if readiness.get("stopping_amendment_id") != stopping.get("amendment_id"):
        raise ValueError("Qualification readiness stopping-amendment identity mismatch")
    if readiness.get("stopping_amendment_sha256") != sha256(stopping_path):
        raise ValueError("Qualification readiness stopping-amendment SHA-256 mismatch")
    for field in (
    ...
    if binding.get("stopping_amendment_id") != stopping.get("amendment_id"):
        raise ValueError("Binding stopping-amendment identity mismatch")
    if binding.get("stopping_amendment_sha256") != sha256(stopping_path):
        raise ValueError("Binding stopping-amendment SHA-256 mismatch")
...
def execute_once(args: argparse.Namespace, checked: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        report = {
            "evaluation_id": "project05-m3star-final-blind-v0.3",
