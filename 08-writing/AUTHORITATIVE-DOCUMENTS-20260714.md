...
状态：论文 v0.9 为唯一母本；采用 Reviewer #1 路线 B。Claim/动作目标 Round 1 失败永久保留，旧粒度结果因来源不独立作废，当前等待第三人裁决、粒度独立重标与 codebook v2

## 1. 唯一写作入口

| 用途 | 当前权威文件 | 说明 |
|---|---|---|
| 论文主稿 | `paper-main-draft-v0.9-route-b-positioning-20260714.md` | 唯一论文母本；按安全 Top-4 文献群重构开篇并执行 Route B 收缩，方法与实验主干不变 |
| Top-4 定位与撞题矩阵 | `paper-top4-positioning-and-gap-synthesis-v0.1-20260714.md` | 当前唯一文献定位依据；不指定单一母文，区分上游构图/对齐与本文调查控制层 |
| Route B 修改承诺 | `reviewer1-route-b-formal-revision-commitment-v0.1-20260714.md` | 六项不可撤销承诺 |
| Route B 构念 Gate | `route-b-construct-gate-execution-protocol-v0.1-20260714.md` | 粒度重标、裁决、Round 2、planner-visible 变更与 C07-C12 重跑规则 |
| 论文写作记录 | `paper-main-authoring-record-v0.8-20260713.md` | 一句话论点、术语、人工结果与未完成门槛 |
...
| 专利主稿 | `patent-main-draft-v0.8-route-b-20260714.md` | 当前唯一专利文字母本；仅方法保护范围不变，将公开意图收缩为版本化预执行公开目标，并排除 IAA 技术效果 |
| 专利交付包 | `patent-package-v0.7/` | 当前格式与图件包；尚未用 v0.8 文字重建，不能覆盖 v0.8 文字母本 |
| 专利范围纠正说明 | `patent-v0.6-method-only-scope-correction-20260713.md` | 已删除系统、设备及存储介质权利要求，v0.5 降为历史稿 |
| 专利格式合同 | `patent-format-contract-v0.1-20260714.md` | 记录两份参考专利的共同版式、继承边界和 v0.7 生成规则；不继承参考稿技术内容 |
...
| 双人盲标首轮结果 | `human-annotation-round1-results-v0.2-route-b-correction-20260714.md` + `../09-experiments/annotation/c07_c11_v0.2/` | 54 个 Claim/动作目标项目来源独立可比且均未过门槛；60 个粒度标签来源不独立，全部作废并等待重标；校准等待 32 项裁决 |
| 第三人盲裁决包 | `../09-experiments/annotation/distribution/c07_c11_v0.2_adjudication_v0.1/` | 7 Claim + 25 Intent；不含 A/B 标签、管理员 key 或规划结果 |
...
| Claim 来源访问台账 | `human-annotation-source-access-ledger-v0.2-20260713.md` | 来源 Gate 的标注前冻结记录；当前人工状态以首轮结果文件为准 |
...
11. 专利 v0.8 仍仅保护方法；系统、设备及存储介质权利要求保持删除状态。预执行公开目标由动作模板、通道能力或调查请求在执行前生成并留痕，不以分析师一致性为实施前提；C12 两级来源 Gate 仍是可选实施例，FTR-010 在完成中国专利补检和代理师审查前不得升级为独立必要特征。
12. C07-C11 A/B 首轮已完成，但 Claim weighted kappa=-0.1455、Intent Jaccard=0.3673/F1=0.4878，均未过预注册门槛；第三人裁决不会改变首轮 IAA。
13. 粒度 A/B 源文件 SHA-256 完全相同；60 个标签全部作废，论文、图表和答辩材料不得展示旧一致性数值。必须使用隔离包重新标注并保留生成、交付、回收时间和双重哈希。
14. 2026-07-14 的论文和专利修改仅是定位及术语收缩，没有改变 planner-visible schema，当前不触发重跑；任何运行字段、标签映射或粒度规则变化均强制重跑 C07-C12 全策略矩阵。
15. 构念 Gate 关闭前不增加 DQN、LLM Agent 或新大数据源。

## 4. 历史文件处理规则

`AUTHORITATIVE-DOCUMENTS-20260713.md` 及更早状态索引、论文 v0.8 及更早稿、专利 v0.7 及更早稿、旧 reviewer/rigor review、早期 contribution brief、C07-C09 协议和早期 M3a 结果均保留为研究过程档案，不删除、不改写其阶段性事实，但不得继续承担当前状态索引或下一步计划的功能。

## 5. 当前未完成门槛

1. 由第三名标注者裁决 7 个 Claim 和 25 个动作目标分歧；使用隔离包重新独立标注 60 个粒度项目，再运行最终代理校准。
2. 根据首轮失败模式冻结 codebook round 2；不得删除困难 item 或用裁决结果改写首轮 IAA。
3. C12 已提供一个更接近运营现场的独立 incident，但仍需更多组织/incident 或分析师效用端点才能主张外部泛化；重复 mask/seed 不得重计为独立攻击。
4. 若主张真实归因能力，必须增加 actor/campaign 正确性或分析师效用终点；当前内部 success 不等于归因准确率。
5. AFA 源码/任务映射已完成；正式投稿若要求数值复现，必须先冻结 endpoint contract，并将跨任务复现或 adapter 与主结果分表。
6. 专利 v0.8 文字母本已完成 Route B 收缩，但交付包尚未按 v0.8 重建；正式提交前仍须由代理师复核中国专利补检，确认 FTR-002 改写方案、权利要求 9 和 FTR-010 的处理，并完成权属、公开日、发明人/申请人确认和逐项审查。
