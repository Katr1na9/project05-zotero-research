# Project05 论文 v0.6 写作记录

日期：2026-07-13

流程：nature-writing → Reviewer major revision → C11 第三封装内部冻结评估 → 原始回指复核 → C11 全策略冻结迁移 → v0.6 边界整合

## 1. 一句话论点

在证据只能部分对齐、动作收益事前不可见、通道可能失效且预算有限时，本文把 APT 归因的前置环节定义为可审计的调查控制问题；C07-C10 支持闭环可执行性和 M2 的局部部署价值，C11 则通过冻结策略排序反转和 AND/OR 敏感性证明方法效用不能脱离任务粒度、证据组合语义和动作空间外推。
...
| frozen policy transfer | 冻结策略迁移 | 在 C11 上重新训练/调参 |
