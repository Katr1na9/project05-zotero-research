# 面向不完整证据 APT 归因的对齐感知证据缺口建模与成本约束主动取证
...
**稿件状态**：Codex 权威主稿 v0.2，2026-07-11

**作者与单位**：待确认

**写作边界**：本文主张问题设定、信息边界、证据缺口状态和可复现闭环，不主张 M3a、XGBoost 或 DQN 已全面优于强规则基线。

## 摘要

高级持续性威胁归因通常建立在网络威胁情报描述与本地遥测证据之间的关联之上。然而在真实调查中，关键日志可能缺失、采集通道可能失效、证据仅能覆盖攻击链的一部分。现有研究主要关注威胁情报抽取、攻击图构建、溯源图对齐或攻击者分类，较少直接回答证据不足时“下一步取什么证据、何时停止、当前最多能归因到什么粒度”。本文将该问题形式化为对齐之后的成本约束主动取证任务。首先，将 CTI 行为图与当前 evidence claims 的部分对齐结果构造成可更新的证据缺口状态，显式记录未覆盖节点、关键缺口、阶段覆盖、预算和可支撑归因粒度。其次，建立公开动作意图与隐藏实际恢复结果相分离的信息边界，并通过采集通道门控使动作收益只在执行后暴露。再次，在统一状态-动作接口下实现覆盖率、条件互信息代理、规则规划器 M1/M2、节点级缺口兼容性 M3a、Logistic、XGBoost、Oracle 和显式 STOP，以区分状态表示、动作设计与策略学习各自的作用。实验由六个开发案例、四个参数锁定真实案例、通道失效与部分可达压力测试以及 192 个可控非短视环境组成。C07-C10 上，M2 与 XGBoost 的序贯成功率均为 1.0000，平均成功成本分别为 4.5333 和 4.8278；M3a 成功率为 0.9500，且在 C10 出现 0.2000 的过早停止。三案例紧预算实验中，M2 在 Oracle 最小成本 (C^*)、(C^*+1) 和 (C^*+2) 下的成功率为 66.67%、66.67% 和 100.00%，均高于 M3a。非短视诊断显示，一步贪心成功率为 0，完整动态规划为 0.7156，证明更深规划具有结构必要性；但完整规划在当前规模下中位耗时仅 0.7692 ms，因此没有启动 DQN 的必要证据。结果表明，本文的可靠增量不是某一学习器的全面胜出，而是把部分对齐转化为可审计、可停止、可受预算约束的调查决策问题，并给出其有效范围与失败边界。
...
POIROT 等工作证明了 CTI 查询图与审计日志图之间行为对齐的价值[1]；EXTRACTOR 与 AttacKG 等工作进一步推动了从自然语言报告中抽取攻击行为和构建 technique graph[2-3]。近年来，TAA-EPLMR、AURA、APT-ATT、APTChaser 和 MM-AttacKG 分别探索了证据路径增强的大模型推理、多智能体知识增强归因、异构威胁情报表示、攻击技术建模和多模态攻击图构建[4-8]。这些研究增强了“已有什么证据时如何表示和推理”的能力，但对另一类决策问题覆盖有限：当现有证据尚不足以支持目标粒度时，系统应该执行哪个取证动作，动作失败后如何更新判断，以及什么时候应当停止而不是继续生成结论。
...
CTI 文本只有被转换为结构化行为表示，才能与本地事件建立可计算联系。EXTRACTOR 从威胁报告中抽取攻击行为[2]，AttacKG 将 technique 及其关系组织为攻击知识图[3]，MM-AttacKG 又把多模态信息与大模型用于攻击图构建[8]。POIROT 则从另一侧出发，把攻击行为查询与内核审计记录进行图匹配[1]。这条研究线回答了“怎样得到图”和“怎样匹配图”，也是本文状态构造的上游基础。本文不替代抽取器或对齐器，而是研究部分对齐之后如何继续采证。

### 2.2 APT 归因与大模型辅助推理

TAA-EPLMR 通过证据路径增强 LLM 的威胁行为者归因推理[4]；AURA 使用知识增强多智能体框架组织归因任务[5]；APT-ATT 结合异构威胁情报表示与 CTGAN 改善闭集归因[6]；APTChaser 基于攻击技术建模进行威胁归因[7]。这些工作说明行为路径、异构表示和语言推理均可提高证据组织能力，但它们的核心输出仍主要是归因判断或候选实体。本文把输出前移为调查控制：当证据只支持较粗粒度结论时，系统先决定补什么证据或停止，不让语言流畅性替代证据充分性。

### 2.3 主动特征获取与非短视规划

主动特征获取研究把每个未观测特征视为具有成本的动作，在预测效用与获取成本之间进行序贯权衡[9]。NOCTA 强调非贪心的长期目标与成本权衡[10]，WinRegRL 则把强化学习用于 Windows 注册表事件响应中的调查规划[11]。这些工作为本文提供了决策理论参照，但网络安全证据采集与通用 AFA 存在三点差异：证据动作可能跨通道失效，动作意图不等于实际可恢复内容，最终输出还有证据支持上限而非仅有分类损失。因此本文需要显式建模通道、来源指针、归因粒度和正当停止。
...
在真正不可达压力中，Oracle 的可达成功率为 0.8148，约 18.52% 条件需要降级；M3a 和 M3b 与 Oracle 的停止判定对齐，证明显式 STOP 能表达“正确认输”。但在保留可靠回退、预算恰好够最短路径的 10 个部分可达条件中，Oracle 成功率为 1.00，静态和自适应 M3b 均约为 0.10，M3a 为 0.00。典型错误是先执行廉价但离线的死通道动作，随后因预算不足而 STOP。该结果表明，会识别不可达不等于会避开诱饵并选择可靠路径。
...
本文针对证据不完整条件下的 APT 归因，提出对齐感知证据缺口状态和成本约束主动取证闭环。系统在不读取隐藏恢复结果的前提下选择下一动作，并以通道反馈、预算、支持粒度和显式 STOP 控制输出。四个参数锁定真实案例表明 M2 仍是当前最稳健的部署锚点，XGBoost 可改善线性对照但尚未超过 M2，M3a 的静态节点兼容性假设在 C10 和紧预算实验中被证伪。非短视诊断进一步证明需要更深规划，但当前没有引入 DQN 的必要。后续工作的优先级应是把轻量非短视规划接入真实案例、建立独立的 claim/意图标注评测，并扩大跨 engagement 案例数量；LLM、多模态和 agent 则作为受 schema 与来源约束的扩展模块，而不是替代证据本身。

## 参考文献（工作稿，提交前由 Zotero 导出样式替换）

[1] Milajerdi S M, Eshete B, Gjomemo R, et al. POIROT: Aligning Attack Behavior with Kernel Audit Records for Cyber Threat Hunting. ACM CCS, 2019. DOI: 10.1145/3319535.3363217.

[2] Satvat K, Gjomemo R, Venkatakrishnan V N. EXTRACTOR: Extracting Attack Behavior from Threat Reports. IEEE EuroS&P, 2021. DOI: 10.1109/EuroSP51992.2021.00046.

[3] Li Z, Zeng J, Chen Y, et al. AttacKG: Constructing Technique Knowledge Graph from Cyber Threat Intelligence Reports. ESORICS, 2022. DOI: 10.1007/978-3-031-17140-6_29.

[4] Xiao N, Lang B, Chen Y, et al. TAA-EPLMR: Threat Actor Attribution via Evidence Path-Enhanced Large Language Model Reasoning. IEEE Big Data, 2025. DOI: 10.1109/BIGDATA66926.2025.11402113.

[5] Rani N, Shukla S K. AURA: A Multi-Agent Intelligence Framework for Knowledge-Enhanced Cyber Threat Attribution. arXiv:2506.10175, 2025.

[6] Cai S, Wang G, Chen J, et al. APT-ATT: An Efficient APT Attribution Model Based on Heterogeneous Threat Intelligence Representation and CTGAN. Computer Networks, 270:111511, 2025. DOI: 10.1016/j.comnet.2025.111511.

[7] Zhang Y, Yang P, Jiang Z, et al. APTChaser: Cyber Threat Attribution via Attack Technique Modeling. ICDF2C 2024 proceedings, 2025:168-185. DOI: 10.1007/978-3-031-89363-6_10.

[8] Zhang Y, Zhao X, Ma Y, et al. MM-AttacKG: A Multimodal Approach to Attack Graph Construction with Large Language Models. arXiv:2506.16968, 2025.

[9] Aronsson L, Rahbar A, Chehreghani M H. A Survey on Active Feature Acquisition Strategies. arXiv:2502.11067, v2, 2026.

[10] Dinh D, Chen B, Qu Y, et al. NOCTA: Non-Greedy Objective Cost-Tradeoff Acquisition for Longitudinal Data. arXiv:2507.12412, 2025/2026 revision.
[11] Ghanem M C, Wojtczak D, Benkhelifa E, et al. Leveraging Reinforcement Learning for an Efficient Windows Registry Analysis during Cyber Incident Response. Scientific Reports, 2026. DOI: 10.1038/s41598-026-57787-6.

## 图表落位清单

- 图 1：CTI-本地证据部分对齐到主动取证闭环。
- 图 2：公开动作意图、隐藏恢复集合与通道执行的信息边界。
- 图 3：M2、M3a、监督学习与非短视规划的统一接口。
- 图 4：C07-C09 紧预算成功率曲线，复用 `fig-budget-efficiency-c07-c09.pdf`。
- 图 5：C07-C10 序贯成本/成功率对比。
- 图 6：非短视 Gate 的解锁链、诱饵动作与策略结果。
