...
**稿件状态**：v0.8；在 v0.7 的 C11/C12 分层结果基础上，补入 C07-C11 双人盲标首轮一致性负结果。A/B 的 114/114 条首轮标签已可比，但 Claim 与 Intent 均未通过预注册一致性门槛；粒度文件因 A/B 源文件哈希相同而暂缓作为独立双盲证据，最终代理校准等待第三人裁决。

## 摘要

真实 APT 调查通常在证据尚不完整时开始，而多数归因与对齐方法主要回答如何解释已经获得的证据。它们较少处理一个部署前置问题：在不知道动作真实收益、采集通道可能失败且预算有限时，下一步应采什么、何时停止，以及当前材料最多支持何种调查结论。本文将部分 CTI—本地证据对齐转化为受信息边界约束的调查控制问题：以可更新证据缺口状态表示调查进度，通过运行时公开视图隔离动作意图与隐藏实际恢复集合，并以预算、通道反馈、结论粒度和显式 STOP 约束动作选择。我们在六个开发案例、四个参数锁定 G3 案例、一个 OTRF APT29 仿真案例和一个从生产 SOC 数据筛出的双流 incident 上评估该接口。C07-C10 中，透明部署策略 M2 在 180/180 个重复条件达到内部目标，平均成功成本为 4.5333；XGBoost、公开 Depth-2、AFA-VOI Myopic 与三步 Rollout 均未降低该成本。C11 在第三种 Windows JSONL/多 provider 封装下把目标截断为 G2，冻结 XGBoost 与 Logistic 的成本均为 3.0667，低于 M2 的 3.6667，而 Depth-2 出现 1 次成功退化。C12 从 13,119 条厂商模板报告出发，经元数据和事件源两级 Gate 只保留具有 ASA 与 Windows AD 原始事件流的一个 G1 案例；所有扩展策略均在 45 个重复条件保持零粒度越界，但 Depth-2 与 Oracle 成本均为 0.8889，M2 为 1.4222，策略排序再次改变。C07-C11 的 114 项双人盲标首轮显示，Claim 支持度加权 kappa 为 -0.1455，公开意图 mean Jaccard 为 0.3673、micro-F1 为 0.4878，均未通过预注册门槛；粒度文件因来源哈希相同而暂缓解释。本文不声称提高了攻击者识别准确率；其贡献是给出一种可审计的调查决策框架，并以跨封装、跨来源的排序反转、语义敏感性和人工复现失败说明策略收益必须在明确的信息、任务与证据组合边界内解释。
...
本文的贡献不以新规划器全面胜出为前提，而分为四点。第一，定义部分对齐之后的调查控制任务，以证据缺口状态统一表示节点/边覆盖、关键缺口、通道反馈、预算和当前可支撑结论粒度。第二，给出公开动作意图与隐藏恢复集合分离的信息边界，并以编译期审计和运行时字段 allowlist 将该边界落实为可测试接口；在此基础上形式化节点级泄漏、证据增加下的粒度单调性和 Bellman 意义下的停止条件。第三，在同一公开接口下适配透明规则、监督学习、通用 AFA-VOI、有限前瞻、完整规划和显式 STOP，并对 AFABench、AACO 与 WinRegRL 的官方代码和任务端点进行冻结审计，明确“领域适配”与“官方同任务复现”的边界。第四，在 DARPA TC E5 与 OpTC 的四个参数锁定 G3 案例、一个 OTRF G2 仿真案例和一个生产 SOC 衍生 G1 案例中实施分层压力测试；C11 与 C12 均改变了 C07-C10 上的策略排序，C12 还暴露出厂商产品标签、原始事件流和相关图投影之间的差异。该组合证据支持可审计调查控制及跨场景策略审计的价值，但不支持任一策略全局最优、真实 actor 识别提升或广泛跨域泛化。
...
主动特征获取（AFA）把每个未观测特征视为具有成本的动作，在预测效用与采集成本之间进行序贯权衡 [@aronsson_survey_2025]。AACO 学习 acquisition-conditioned oracle 以支持非贪心子集选择，NOCTA 以候选未来采集计划的预期预测损失和成本构造非贪心目标，AFABench 则提供带 STOP、非均匀成本和统一方法注册的通用评测框架 [@valancius_acquisition_2024; @noauthor_nocta_2025; @schutz_afabench_2026]。WinRegRL 在专家指定的 Windows 取证 MDP 上使用动态规划和有限 RL 修正 [@ghanem_leveraging_2026]。这些工作已覆盖“有成本地选择下一观测”的宽泛问题，因此本文不把 AFA、MDP 或非贪心规划本身视为创新。差异位于部署信息结构：静态特征揭示不等于可能失败的证据 bundle 执行，公开意图不能等同于实际可恢复内容，输出还要受来源可回指性和结论粒度上限约束。为检验这些差异是否实质存在，本文把一步 AFA-VOI 与三步 objective-cost rollout 适配到同一动作空间，并冻结审计三套可得官方/作者代码，而不是只在相关工作中做文字区分。
...
| 本文 | CTI—本地证据缺口、通道、预算、反馈 | 是 | **是** | **是；首轮人工复现未过门槛，最终校准待裁决** |
...
### 6.10 C07-C11 双人盲标首轮结果

C07-C11 的首轮人工包包含 27 个 Claim、27 个公开动作意图和 60 个粒度状态。两名标注者均提交全部项目；导入器在不覆盖源文件的条件下记录六个 SHA-256，并对 12 处可唯一解释的 CSV 分隔或标签拼写错误进行机械规范化。规范化后 114/114 项均通过 blind ID、候选节点和冻结标签域检查。

| 任务 | 双人可比 | 原始/Exact 一致 | Weighted kappa | Mean Jaccard | Micro F1 | 分歧项 |
|---|---:|---:|---:|---:|---:|---:|
| Claim 支持度 | 27 | 0.7407 | -0.1455 | - | - | 7 |
| 公开动作意图 | 27 | 0.0741 | - | 0.3673 | 0.4878 | 25 |
| 可支撑粒度 | 60 | 1.0000 | 1.0000 | - | - | 0 |

Claim κ 和公开意图的 Jaccard/F1 均未达到预注册门槛。Claim 标签高度集中于 `1_partial`，边际失衡使 κ 明显低于原始一致率；来源指针虽为 27/27 原始一致，但两人均只使用 `yes`，没有负例可检验错误指针识别。Intent 的 25/27 分歧主要反映对“单个直接目标”与“宽意图候选集合”的不同理解，因此当前公开意图 codebook 尚不可复现。

粒度的数值结果暂不作为人工一致性证据：A/B 两个源文件 SHA-256 完全相同且自由文本逐字节一致。在确认两份文件确由两名标注者独立完成之前，不能把程序输出的 1.0000 解释为双盲可靠性。第三人裁决包现包含 7 个 Claim 和 25 个 Intent 分歧项，不含 A/B 答案或管理员映射；最终 human-vs-proxy 校准保持 `awaiting_adjudication`。裁决可产生最终标签，但不会把未过门槛的首轮一致性改写为通过。
