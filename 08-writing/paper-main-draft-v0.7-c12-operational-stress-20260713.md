...
**稿件状态**：v0.7；在 v0.6 的 C11 冻结迁移基础上，补入外部 AFA 官方源码与任务接口审计，以及 WitFoo Precinct6 生产 SOC 数据上的 C12 双流事件压力。C11 始终作为单独的 G2 仿真压力，C12 始终作为单独的 G1 运营数据压力，二者均不并入 C07-C10 的 G3 主结果均值。

## 摘要

真实 APT 调查通常在证据尚不完整时开始，而多数归因与对齐方法主要回答如何解释已经获得的证据。它们较少处理一个部署前置问题：在不知道动作真实收益、采集通道可能失败且预算有限时，下一步应采什么、何时停止，以及当前材料最多支持何种调查结论。本文将部分 CTI—本地证据对齐转化为受信息边界约束的调查控制问题：以可更新证据缺口状态表示调查进度，通过运行时公开视图隔离动作意图与隐藏实际恢复集合，并以预算、通道反馈、结论粒度和显式 STOP 约束动作选择。我们在六个开发案例、四个参数锁定 G3 案例、一个 OTRF APT29 仿真案例和一个从生产 SOC 数据筛出的双流 incident 上评估该接口。C07-C10 中，透明部署策略 M2 在 180/180 个重复条件达到内部目标，平均成功成本为 4.5333；XGBoost、公开 Depth-2、AFA-VOI Myopic 与三步 Rollout 均未降低该成本。C11 在第三种 Windows JSONL/多 provider 封装下把目标截断为 G2，冻结 XGBoost 与 Logistic 的成本均为 3.0667，低于 M2 的 3.6667，而 Depth-2 出现 1 次成功退化。C12 从 13,119 条厂商模板报告出发，经元数据和事件源两级 Gate 只保留具有 ASA 与 Windows AD 原始事件流的一个 G1 案例；所有扩展策略均在 45 个重复条件保持零粒度越界，但 Depth-2 与 Oracle 成本均为 0.8889，M2 为 1.4222，策略排序再次改变。本文不声称提高了攻击者识别准确率；其贡献是给出一种可审计的调查决策框架，并以跨封装、跨来源的排序反转和语义敏感性说明策略收益必须在明确的信息、任务与证据组合边界内解释。
