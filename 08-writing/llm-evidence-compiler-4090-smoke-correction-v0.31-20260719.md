...
日期：2026-07-19
状态：实现修复与本地验证已完成；用户已授权一次纠正后的 smoke，待服务器执行
...
第一次启动因实现错误在 forward 前中止，不计为有优化器的 smoke 执行；整个阶段仍最多允许一次产生 forward/backward 和 1 个 optimizer step 的 smoke。纠正后的 smoke 若失败，不得自动再试。
