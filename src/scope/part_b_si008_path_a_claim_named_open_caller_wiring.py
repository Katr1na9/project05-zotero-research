"""Test-only Path A caller for the PB-SI-008 CLAIM named-open gate.
...
from src.scope.part_b_si008_named_open_path_a_claim_candidacy import (
...
class PathAClaimCallerWiringDenied(ValueError):
    """Fail-closed rejection before the CLAIM gate is invoked."""
...
def evaluate_path_a_claim_structural_binding_for_named_open(
    caller_input: Mapping[str, Any],
    *,
    test_only_authority: Mapping[str, Any],
) -> dict[str, object]:
    """Build the exact request and return the CLAIM gate record unchanged."""
    ...
    return evaluate_si008_named_open_claim_request(request)
...
def _require_test_only_authority(
    ...
    if (
        ...
        raise PathAClaimCallerWiringDenied(
...
__all__ = [
    ...
    "PathAClaimCallerWiringDenied",
    "TEST_ONLY_AUTHORITY_FIELDS",
    "evaluate_path_a_claim_structural_binding_for_named_open",
]
