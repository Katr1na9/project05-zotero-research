"""Additive PB-SI-008 gate for one named Path A evidence target.

Path A readonly GREEN MUST NOT be inferred as L2 PASS, Part B PASS, or
unrestricted Part B elevation.

Legacy exact-four-field requests are delegated to the protected dual-track
deny runtime. Named exact-eighteen-field requests are classified from their
declarations only; package and receipt references are never dereferenced.
"""
...
ALLOW_DECISION = "ALLOW_NAMED_EVIDENCE_CANDIDACY_ONLY"
...
DENIED_PROMOTION_TARGETS = frozenset(
...
AUTHORITY_BOUNDARY = {
    ...
    "named_evidence_candidacy_classification_authority": False,
}
ADJACENT_GATES = {
...
def evaluate_si008_named_open_request(
    request: Mapping[str, Any],
) -> dict[str, object]:
    """Evaluate one legacy or named request without dereferencing references."""

    if isinstance(request, Mapping) and set(request) == LEGACY_REQUEST_FIELDS:
        return evaluate_dual_track_request(request)

    if not isinstance(request, Mapping):
        return _named_record(
            {},
            decision="DENY",
            reason_code="SI008-NAMED-003_REQUEST_NOT_QUALIFIED",
            qualified=False,
        )
    ...
    if not _has_exact_named_types(request):
        return _named_record(
            request,
            decision="DENY",
            reason_code="SI008-NAMED-003_REQUEST_NOT_QUALIFIED",
    ...
    if promotion_target in DENIED_PROMOTION_TARGETS:
        return _named_record(
            request,
            decision="DENY",
            reason_code=(
    ...
    if not _qualifies_for_named_evidence_candidacy(request):
        return _named_record(
            request,
            decision="DENY",
            reason_code="SI008-NAMED-003_REQUEST_NOT_QUALIFIED",
    ...
    return _named_record(
        ...
        reason_code="SI008-NAMED-001_EXACT_TARGET_EVIDENCE_CANDIDACY",
        qualified=True,
    )


def _has_exact_named_types(request: Mapping[str, Any]) -> bool:
...
def _qualifies_for_named_evidence_candidacy(
    request: Mapping[str, Any],
) -> bool:
    for field, expected in _EXACT_NAMED_VALUES.items():
    ...
    if any(
        ...
        return False
    pair = REFERENCE_PAIRS.get(str(request.get("reference_kind")))
    if pair is None:
        return False
    return all(request.get(field) == expected for field, expected in pair.items())


def _normalized_named_request(
    request: Mapping[str, Any],
) -> dict[str, object]:
...
def _named_record(
    ...
    *,
    ...
    normalized = _normalized_named_request(request)
    identity = {
        "request": normalized,
    ...
    record: dict[str, object] = {
        ...
        "reason_code": reason_code,
        "named_target_id": NAMED_TARGET_ID,
        "allowed_promotion_target": "EVIDENCE",
